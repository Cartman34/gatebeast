...
- **Description propre à une valeur ou une forme** — optionnelle : une entrée peut décrire à part une valeur ou une forme précise, introduite par la formule fixe « Description propre à la valeur `X` » ou « Description propre à la forme `X` », suivie de sa propre description en italique. Cette formule est un repère de mise en forme de l'entrée, jamais un mot que la description choisit : c'est elle, et elle seule, qui distingue une description à citer pour cette valeur d'un texte explicatif ordinaire.
- **AUCUN DÉTAIL NE DESCEND SOUS CINQ PIXELS À L'ÉCRAN — c'est le critère de lisibilité, et il commande les dimensions écrites.** Une case vaut vingt-quatre pixels dans le jeu : un détail décrit comme large d'un cinquième de case en fait donc **cinq**, et disparaît. Constaté sur l'herbe de clairière — neuf touffes d'un cinquième de case, invisibles à l'échelle du monde, alors que l'image de près était juste. On décrit un sujet **tel qu'il se verra dans le jeu**, jamais tel qu'on le regarde agrandi : mieux vaut peu d'éléments francs que beaucoup de fins.
- **Une description propre COMPLÈTE la description du sujet ; elle ne la remplace que si son variant change la nature de la pièce** (`defines_kind`, [sujets et variants](../assets/sujets-et-variantes.md)). C'est le cas ordinaire qui commande : trois densités d'une même herbe ne diffèrent que par un nombre de touffes, donc le sujet se décrit **une fois** et chaque densité n'écrit que sa quantité. Le portillon, lui, remplace : ce n'est pas une clôture avec une option. Écarté : faire remplacer systématiquement — il a fallu réécrire l'herbe entière trois fois pour un seul nombre qui changeait, trois textes à tenir en accord, et les deux qui n'étaient pas lus sont sortis identiques au troisième.
- **Une description propre ne renvoie jamais à une autre description** — ni « la même herbe que ci-dessus », ni « comme le sujet de base ». Le générateur ne reçoit que ce que la consigne lui donne : un renvoi à un texte qu'il n'a pas sous les yeux ne désigne rien pour lui. Ce qui doit lui parvenir s'écrit, ou se joint en image de référence.
